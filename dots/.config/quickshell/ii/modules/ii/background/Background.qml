...
Scope {
    ...
    property list<real> visualizerPoints: Array.from({
        length: 50
    }, () => 0)
    ...
    Process {
        ...
        onRunningChanged: {
            ...
            if (!cavaProc.running)
                root.visualizerPoints = Array.from({
                    length: 50
                }, () => 0);
        }
    ...
    Variants {
        ...
        PanelWindow {
            ...
            property var wallpaperData: WallpaperListener.effectivePerMonitor[monitor.name] || {
            ...
            Item {
                ...
                Image {
                    ...
                    smooth: true
                    asynchronous: true
                    // Only enable layer (GPU texture) while a transition is running;
                    // keeping it on permanently wastes a full-screen VRAM allocation.
                ...
                StyledImage {
                    ...
                    property bool usePerMonitorRange: WallpaperListener.multiMonitorEnabled && (wallpaperData.workspaceFirst !== undefined && wallpaperData.workspaceLast !== undefined)
